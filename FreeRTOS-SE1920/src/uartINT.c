...
#define UART_FCR_TRG_LEV0 0b00000000
#define UART_FCR_TRG_LEV1 0b01000000
#define UART_FCR_TRG_LEV2 0b10000000
#define UART_FCR_TRG_LEV3 0b11000000
...
#define UART_IER_RBRINT_EN 0b1
#define UART_IER_THREINT_EN 0b10
#define UART_IER_RLSINT_EN 0b100
#define UART_IER_BITMASK 0b1100000111

#define UART_TER_TXEN 0b10000000

#define UART_IIR_INTID_MASK (0b111 << 1)
#define UART_IIR_INTID_RLS (0b011 << 1)
#define UART_IIR_INTID_RDA (0b010 << 1)
#define UART_IIR_INTID_CTI (0b110 << 1)
#define UART_IIR_INTID_THRE (0b001 << 1)
#define UART_IIR_INTSTAT_PEND 0b1

LPC_UART_TypeDef *UARTx = LPC_UART2;

UART_RBUF_Type rbuffer;

bool UART_WriteToFIFO = true;
...
{
	rbuffer.rxRead = rbuffer.rxWrite;
}

void UARTINT_EmptyRBufferTx()
{
	rbuffer.txWrite = rbuffer.txRead;
}


/*bool UARTINTSetDivisors(unsigned int baud)
{
	if( baud > 0xFFFFFFFF )
		return false;
	//switch (baud){
	...
	//}
	UARTx -> FDR = 1<<4;
	int res = SystemCoreClock/(baud<<4);
	UARTx -> DLL = res & 0xFF;
	UARTx -> DLM = (res>>8) & 0xFF;
	return true;
}*/

bool UARTINTSetDivisors(unsigned int baud, uint32_t pclock)
{
...
bool UARTINT_Initialize(unsigned int baud) {
	...
	if (!UARTINTSetDivisors(baud, pclk))
		return false;
	...
	// Priority - NVIC_SetPriority
	IRQn_Type UARTx_IRQn = UART2_IRQn;
	WAIT_Ms(10);
...
uint32_t UARTINT_ReadBuffer(uint8_t *buffer, uint32_t len, int timeOut) {
	Counter *cTimer = COUNTER_Create();

	uint8_t *data = (uint8_t*) buffer;
	uint32_t bytes = 0;
	//UARTx->IER &= (~UART_IER_RBRINT_EN) & UART_IER_BITMASK;
	while ((len > 0) /*&& (!(RBUF_IS_EMPTY(rbuffer.rxWrite, rbuffer.rxRead)))*/) {
		if(!(RBUF_IS_EMPTY(rbuffer.rxWrite, rbuffer.rxRead))){
			*data = rbuffer.rx[rbuffer.rxRead];
			data++;
			RBUF_INCR(rbuffer.rxRead);
			bytes++;
			len--;
			COUNTER_Restart( cTimer );
		}
		else
			WAIT_Ms(timeOut/10);
		if( COUNTER_Check( cTimer, timeOut) > 0 ) {
			*data = '\0';
			break;
		}
	}
	UARTx->IER |= UART_IER_RBRINT_EN;
	COUNTER_Destroy(cTimer);
	return bytes;
}
...
uint32_t UARTINT_WriteBuffer(unsigned char *buffer, int len) {
	uint8_t *data = (uint8_t*) buffer;
	uint32_t bytes = 0;
	UARTx->IER &= (~UART_IER_THREINT_EN) & UART_IER_BITMASK; //remove THRE interrupt


	while ((len > 0) && (!RBUF_IS_FULL(rbuffer.txWrite, rbuffer.txRead)))
	{
	...
	if(UART_WriteToFIFO)
	{
		UARTx->THR = rbuffer.tx[rbuffer.txRead];
...
void UART2_IRQHandler(void) {
	...
	while (!((iir = UARTx->IIR) & UART_IIR_INTSTAT_PEND)) {
		...
		case UART_IIR_INTID_RLS:
			...
			//LCDText_Printf("UART ERROR!\nCODE: %d", (int)lsr);
			break;
		case UART_IIR_INTID_RDA:
		case UART_IIR_INTID_CTI:
			uint8_t temp_data = UARTx->RBR;
			while ((UARTx->LSR & UART_LSR_RDR) != 0 && !RBUF_IS_FULL(rbuffer.rxWrite, rbuffer.rxRead)) {
				rbuffer.rx[rbuffer.rxWrite] = temp_data;
		...
		case UART_IIR_INTID_THRE:
			...
			UART_WriteToFIFO = RBUF_IS_EMPTY(rbuffer.txWrite, rbuffer.txRead);
